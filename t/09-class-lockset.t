...
sub nodes {
    ...
    cmp_ok(TLG::TXN->get_lockset->lock_count, '==', 1, '1 lock');
    diag('set');

    ok($node->set( $myns1 => title => $note->{title} ), 'set title');
    cmp_ok(TLG::TXN->get_lockset->lock_count, '==', 4, '4 lock');
    ok($node->set( $myns1 => content => $note->{content} ), 'set content');
    cmp_ok(TLG::TXN->get_lockset->lock_count, '==', 7, '7 lock');
    ok($node->set( $myns2 => score => $note->{score} ), 'set score');
    cmp_ok(TLG::TXN->get_lockset->lock_count, '==', 9, '9 lock');
    ok($node->set( $myns1 => html => $note->{html} ), 'set html');
    cmp_ok(TLG::TXN->get_lockset->lock_count, '==', 12, '12 lock');
    ...
    diag('query 2 node');
    my $lockn = TLG::TXN->get_lockset->lock_count;
    TLG::TXN->pause;
    ...
    TLG::TXN->resume;
    cmp_ok(TLG::TXN->get_lockset->lock_count, '==', $lockn, 'TXN paused, no additional lock');
